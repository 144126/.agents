...
async function call(model,content){
  ...
  const r=await fetch(base+'/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:m,messages:[{role:'user',content}],max_tokens:8192,temperature:0.7})})
  const t=await r.text()
  if(!r.ok) die(`LLM ${t.slice(0,800)} (model=${m})`)
...
async function run(file,model){
  ...
  while(true){
    ...
    const raw=await call(model,prompt(p,a[i]))
...
async function main(){
  ...
  if(!args.length||args.includes('-h')||args.includes('--help')){console.log('rethink "<topic>" N [--angle "s"]... [--model id]\nrethink <file.r> [--model id]');process.exit(0)}
  let model=process.env.RETHINK_MODEL||'openrouter/z-ai/glm-5.3-flash'
  const angles=[];const rest=[]
  for(let i=0;i<args.length;i++){const a=args[i];if(a==='--model')model=args[++i]||die('--model needs id');else if(a==='--angle')angles.push(args[++i]||die('--angle needs text'));else if(a==='--fast'||a==='--reasoning'){if(args[i+1]&&!args[i+1].startsWith('-'))i++}else rest.push(a)}
  const first=rest[0];if(!first) die('need <topic> N or <file.r>')
  if(first.endsWith('.r')||existsSync(resolve(first))){await run(resolve(first),model);return}
  ...
  if(existsSync(file)){console.log(`resume ${file}`);await run(file,model);return}
  save(file,topic,list,list.map(()=>0));console.log(`init → ${file} (${n} leaves)`);await run(file,model)
}
